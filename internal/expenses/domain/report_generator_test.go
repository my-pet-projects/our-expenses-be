...
func TestGenerateByDateReport(t *testing.T) {
	...
	if firstCategoryByDate.Date == date1 {
		...
		assert.True(t, expectedTotal.OriginalTotal.Equal(firstCategoryByDate.GrandTotal.SubTotals["EUR"].OriginalTotal))
		...
		for _, catLevel1 := range firstCategoryByDate.SubCategories {
			if catLevel1.Category.ID() == category1.ID() {
				...
				assert.True(t, expectedTotal.OriginalTotal.Equal(catLevel1.GrandTotal.SubTotals["EUR"].OriginalTotal))
				...
				for _, catLevel2 := range catLevel1.SubCategories {
					if catLevel2.Category.ID() == category11.ID() {
						...
						assert.True(t, expectedTotal.OriginalTotal.Equal(catLevel2.GrandTotal.SubTotals["EUR"].OriginalTotal))
						...
						for _, catLevel3 := range catLevel2.SubCategories {
							if catLevel3.Category.ID() == category111.ID() {
								...
								assert.True(t, expectedTotal.OriginalTotal.Equal(catLevel3.GrandTotal.SubTotals["EUR"].OriginalTotal))
